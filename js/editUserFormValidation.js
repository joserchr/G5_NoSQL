$(document).ready(function() {
    ...
    $("#nuevaContraseña").on("blur", function() {
        validateNewPassword();
    });
    ...
    function validateNewPassword() {
        ...
        if (!regex.test(newPassword) || newPassword.trim() === "") {
            $("#nuevaContraseña").addClass("is-invalid");
            $("#nuevaContraseña-error").show();
            return false;
        } else {
            $("#nuevaContraseña").removeClass("is-invalid");
            $("#nuevaContraseña-error").hide();
            return true;
        }
    }
    ...
    // editUserPassword de envío del formulario
    $("#editUserPassword").on("submit", function(event) {
        event.preventDefault(); // Evita el envío del formulario
        ...
        if (validatePassword() && validateNewPassword() && validateConfirmPassword()) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Registro exitoso, mostrar modal de éxito y redireccionar
                        $("#actualizacionContraseñaExitosaModal").modal("show");
                        setTimeout(function() {
                            window.location.reload();
                        }, 2000); // Redireccionar después de 2 segundos
                    } else {
                        // Registro fallido, mostrar modal de error
                        $("#actualizacionContraseñaFallidaModal").modal("show");
                    }
                },
                error: function(xhr, status, error) {
                    // Manejar errores de conexión o de otro tipo
